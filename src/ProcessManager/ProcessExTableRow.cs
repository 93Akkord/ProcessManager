...
namespace Akkd.ProcessManager {
    public class ProcessExTableRow {
        public string ProcessName { get; }
        public int ProcessId { get; }
        public int? ParentProcessId { get; }
        private ProcessEx ParentProcess { get; }
        public string MemoryUsed { get; }
        public string GroupMemoryUsed { get; }
        private IntPtr Handle { get; }
        public string User { get; }
        public string Arch { get; }
        public string FileName { get; }
        public string CommandLine { get; }
        public long MemoryUsedNum { get; }
        public long GroupMemoryUsedNum { get; }

        private List<ProcessEx> ChildProcesses { get; }
        private string MemoryUsed2 { get; }

        public ProcessExTableRow(ProcessEx process) {
            ...
            MemoryUsed = ProcessEx.ToString(process.MemoryUsed).PadLeft(9, ' ');
            //MemoryUsed = $"{ProcessEx.ToString(process.MemoryUsed)} ({process.MemoryUsed})";
            //MemoryUsed = $"{process.MemoryUsed} ({ProcessEx.ToString(process.MemoryUsed)})";

            MemoryUsed2 = ProcessEx.ToString(process.MemoryUsed2).PadLeft(9, ' ');

            GroupMemoryUsed = ProcessEx.ToString(process.ProcessGroupMemoryUsed).PadLeft(9, ' ');
            //GroupMemoryUsed = $"{ProcessEx.ToString(process.ProcessGroupMemoryUsed)} ({process.ProcessGroupMemoryUsed})";
            //GroupMemoryUsed = $"{process.ProcessGroupMemoryUsed} ({ProcessEx.ToString(process.ProcessGroupMemoryUsed)})";

            MemoryUsedNum = process.MemoryUsed;
            GroupMemoryUsedNum = process.ProcessGroupMemoryUsed;
        }

        public static ProcessExTableRow From(ProcessEx process) {
            return new ProcessExTableRow(process);
        }
    }
    ...
    public class ConsoleTable {
        ...
        public void Write(Format format = Akkd.ProcessManager.Format.Default) {
            switch (format) {
                case Akkd.ProcessManager.Format.Default:
                    Options.OutputTo.WriteLine(ToString());
                    break;
                case Akkd.ProcessManager.Format.MarkDown:
                    Options.OutputTo.WriteLine(ToMarkDownString());
                    break;
                case Akkd.ProcessManager.Format.Alternative:
                    Options.OutputTo.WriteLine(ToStringAlternative());
                    break;
                case Akkd.ProcessManager.Format.Minimal:
